...
export class EventManager {
   ...
   */
  updateScheduledEvents(deltaTime) {
  ...
  matchesCommand(command, event) {
    ...
    // Check preposition
    if (event.trigger.preposition && event.trigger.preposition !== command.preposition) {
      return false;
    }
    
    // Check indirect object
    if (event.trigger.indirectObject && event.trigger.indirectObject !== command.indirectObject) {
      return false;
    }
  ...
  registerDefaultHandlers() {
    ...
    // Look handler
    this.registerHandler('LOOK', (action) => {
      const currentRoom = this.gameState.getCurrentRoom();
      if (currentRoom) {
    ...
    // Inventory handler
    this.registerHandler('INVENTORY', (action) => {
