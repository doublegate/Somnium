...
describe('GameState', () => {
  ...
  describe('Resource Loading', () => {
    ...
    test('should throw error on invalid room data', () => {
      ...
      expect(() => gameState.loadResources(invalidData)).toThrow(
        'Invalid game data'
      );
    });
  });
  ...
  describe('Validation', () => {
    ...
    test('should catch invalid exit references', () => {
      ...
      expect(result.errors).toContain(
        'Room room1 exit north points to non-existent room nonexistent'
      );
    });
    ...
    test('should catch invalid state references', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(
        'State references non-existent room badRoom'
      );
      expect(result.errors).toContain(
        'State inventory contains non-existent item badItem'
      );
    });
  });
});
