...
- **Save/Load**: Local StorageStorage + JSON export
...
**Node Types:**
1. **Item Required** 🔑: Player must have specific item
2. **Action Required** ⚡: Player must perform action
3. **Sequence** 🔢: Steps must be completed in order
4. **Condition** ❓: Check game state before proceeding
5. **Combine Items** 🔗: Combine two items to create new item
6. **Trigger Event** 🎯: Fire game event when completed
...
**Node Types:**
1. **Greeting** 👋: Initial NPC greeting
2. **Question** ❓: NPC asks player something
3. **Response** 💬: NPC responds to player
4. **Branch** 🌳: Conditional branching point
5. **Trade** 💰: Trading interface node
6. **End** 🏁: Conversation termination
...
## 🗂️ Asset Library System (`js/AssetLibrary.js`)
...
## 🏆 Expanded Achievements System (`js/ExpandedAchievements.js`)
...
**Rarity Tiers:**
- **Common** 🥉: Easy to obtain (50 XP reward)
- **Rare** 🥈: Moderate challenge (150 XP reward)
- **Epic** 🥇: Significant accomplishment (300 XP reward)
- **Legendary** 💎: Extremely difficult (500 XP reward)
...
│   └── editors/
│       ├── WorldEditor.js      # ~1000 lines
│       ├── PuzzleBuilder.js    # ~800 lines
│       └── DialogueEditor.js   # ~1000 lines
├── scripts/                    # NEW
│   ├── create-source-icon.js
│   └── generate-icons.js
└── assets/icons/               # NEW
...
*Version 2.1.0 transforms Somnium from a game into a complete game creation platform. Build, share, and play unlimited adventures!* 🎮✨
